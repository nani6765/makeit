...
function PostPage(props) {
  ...
  useEffect(() => {
    ...
    axios.post("/api/community/length", body).then((response) => { //"/api/community/length"로 카테고리 정보를 담은 데이터 전송
      if (response.data.success) {                                  // 서버에서 카테고리에 맞는 게시글 수 검색
  ...
  useEffect(() => { //전체 페이지 수 바뀌면 실행 pageToTalArray에 페이지 수만큼 배열
    let temp = [];
  ...
  useEffect(() => { //카테고리, 정렬 기준 바뀌면 실행, 각 기준에 맞는 1페이지로 이동
    setpageIdx(1);
  }, [props.category, props.sortPost]);

  useEffect(() => { //페이지 바뀌면 스크롤이 가장 위로 이동
    window.scrollTo({
      top: 150,
      left: 0,
      behavior: "smooth",
  ...
  return (
    <>
      ...
      <PageUL> 
        { //페이지 목록 보여주는 태그
        pageTotalArray.map((num, idx) => { //num은 현재 보고 있는 값, idx는 현재 인덱스
          return (
            <li key={idx}>
