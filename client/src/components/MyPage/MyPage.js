...
function MyPage() {
  ...
  const SwitchTaps = () => {
    switch (Taps) {
      ...
      case "알림":
        return <AlarmCenter AlarmType={AlarmType} setAlarmType={setAlarmType} />;
      default:
        return <BasicMyPage setTaps={setTaps} />;
    }
  };
    ...
    switch (Taps) {
      ...
      case "프로필":
        return <React.Fragment>
          <p>프로필 관리</p>;
          <span onClick={() => { setTaps("내정보")}}>X</span>
        </React.Fragment>;

       case "알림":
        return <React.Fragment>
          <div>
            <p className={AlarmType==="알림센터" ? 'active' : null} onClick={() => setAlarmType("알림센터")}>알림센터</p>
            <p className={AlarmType==="쪽지함" ? 'active' : null} onClick={() => setAlarmType("쪽지함")}>쪽지함</p>
          </div>
          <span onClick={() => { setTaps("내정보")}}>X</span>
        </React.Fragment>;
