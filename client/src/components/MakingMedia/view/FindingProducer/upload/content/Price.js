...
function Price(props) {
  ...
  return (
    <PriceDiv>
      ...
      <label>
        <input type="radio" name="price" value="가격선택" defaultChecked={true}/>
        가격선택
      </label>
      <DropdownButton title={props.PriceInfo !== "가격문의" ? props.PriceInfo : "가격선택"}>
      ...
      {InputFlag && (
        <input name="priceInput" placeholder={props.PriceInfo} onChange={(e) => setPriceInput(e.currentTarget.value)} />
      )}
      <label>
        <input type="radio" name="price" value="가격문의" onClick={() => props.setPriceInfo("가격문의")}/>
        가격문의
      </label>
      <FooterBtnArea
