...
function RequestVideoUpload(props) {
  ...
  const submitHandler = (e) => {
    ...
    axios
      .post("/api/making/requestVideo/reqPostSubmit", body)
      .then((response) => {
        if (response.data.success) {
          alert("의뢰 게시가 완료되었습니다.");
          props.history.push("/making?category=영상 의뢰하기&sort=인기순&subCategory=전체&pIdx=0")
        } else {
  ...
  return (
    <>
      <UploadHead>
        <div>
          <h1>
            <span onClick={() => props.history.push("/making?category=영상 의뢰하기&sort=인기순&subCategory=전체&pIdx=0")}>&lt;</span>
            의뢰 작성하기
          </h1>
        </div>
      </UploadHead>
