...
function PartDetail(props) {
    ...
    const setHeader = () => {
        ...
        switch(PostInfo.type) {
            case "FA":
                return <h1 onClick={() => props.history.push({pathname:"/participate", state: {category: "배우찾기"}})}>&lt; 배우 찾기</h1>;
            case "FP":
                return <h1 onClick={() => props.history.push({pathname:"/participate", state: {category: "파트너찾기"}})}>&lt; 파트너 찾기</h1>;
            case "IP":
                return <h1 onClick={() => props.history.push({pathname:"/participate", state: {category: "프로알리기"}})}>&lt; 프로 알리기</h1>;
            case "Lo":
                return <h1 onClick={() => props.history.push({pathname:"/participate", state: {category: "로케이션"}})}>&lt; 로케이션</h1>;
        }
    }

    useEffect(() => {
        let body = {
            postNum: props.match.params.url,
        };
    ...
    return (
        <>
            ...
            {PostInfo.postNum != undefined && <PartDetailContent PostInfo={PostInfo} />}
            {TotalSize > 0 ? (
                <>
                <RepleBtnDiv>
            ...
            {PostInfo._id != null ? (
                <RepleUpload postInfo={PostInfo} type={PostInfo.type} category="/participate/post"/>
            ) : null}
        </>
    )
}
